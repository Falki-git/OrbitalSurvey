...
namespace OrbitalSurvey.Utilities
{
	...
	public class Patches
	{
		...
		private static void RemoveOabExperimentDescriptions(List<OABPartData.PartInfoModuleEntry> __result,
			Data_ScienceExperiment __instance)
		{
			LocalizedString visualMappingExperimentName =
				"OrbitalSurvey/Experiments/OabDescription/DisplayName/VisualMapping";
			LocalizedString biomeMappingExperimentName =
				"OrbitalSurvey/Experiments/OabDescription/DisplayName/BiomeMapping";
			...
			for (int i = 0; i < __result.Count; i++)
			{
				...
				while (i < __result.Count && __result[i].DisplayName.Contains(visualMappingExperimentName))
				{
					...
					// check if this is the first experiment; if it is we leave it intact
