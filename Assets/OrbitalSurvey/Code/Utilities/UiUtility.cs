...
namespace OrbitalSurvey.Utilities
{
    ...
    {
        public static Vector2 GetAdjustedCanvasCoordinatesFromLocalPosition(Vector2 localPosition, float canvasWidth,
            float canvasHeight)
        {
            var mapPositionPercentage = new Vector2(localPosition.x / canvasWidth, localPosition.y / canvasHeight);

            return GetAdjustedCanvasCoordinatesFromMapPositionPercentage(mapPositionPercentage, canvasWidth,
                canvasHeight);
        }
        ...
        {
            var scaledDistanceToCenter = ScaledDistanceToCenter(mapPositionPercentage.x, mapPositionPercentage.y,
        ...
        private static Vector2 ScaledDistanceToCenter(float percentX, float percentY, float canvasWidth,
            float canvasHeight)
        {
        ...
        public static (double latitude, double longitude) GetGeographicCoordinatesFromPositionPercent(double percentX,
            double percentY)
        {
