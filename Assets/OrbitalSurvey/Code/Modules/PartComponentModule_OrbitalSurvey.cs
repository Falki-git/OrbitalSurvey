...
namespace OrbitalSurvey.Modules
{
    public class PartComponentModule_OrbitalSurvey : PartComponentModule
    {
        ...
        {
            Logger.LogDebug($"OnStart triggered. Vessel '{Part?.PartOwner?.SimulationObject?.Vessel?.Name ?? "n/a"}'");

            if (!DataModules.TryGetByType<Data_OrbitalSurvey>(out _dataOrbitalSurvey))
            {
                Logger.LogError("Unable to find a Data_OrbitalSurvey in the PartComponentModule for " +
                                base.Part.PartName);
                return;
            }
                ...
                (DataDeployable?.IsExtended ?? true))
            {
                ...
                if (DebugUI.Instance.DebugFovEnabled != _isDebugCustomFovEnabled)
                {
                    _dataOrbitalSurvey.ScanningStats.FieldOfView = DebugUI.Instance.DebugFovEnabled
                        ? _dataOrbitalSurvey.ScanningFieldOfViewDebug.GetValue()
                        : _dataOrbitalSurvey.ScanningFieldOfView.GetValue();
        ...
        private void PerformRetroactiveScanningIfNeeded(VesselComponent vessel, string body, MapType mapType,
        ...
        {
            Logger.LogDebug("OnFinalizeCreation triggered.");
        }
    }
}