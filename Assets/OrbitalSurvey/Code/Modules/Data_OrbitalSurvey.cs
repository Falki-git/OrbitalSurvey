...
namespace OrbitalSurvey.Modules
{
    ...
    public class Data_OrbitalSurvey : ModuleData
    {
        ...
        [LocalizedField("PartModules/OrbitalSurvey/Status")] [PAMDisplayControl(SortIndex = 1)]
        public ModuleProperty<string> Status = new("Idle");

        [KSPState] [LocalizedField("PartModules/OrbitalSurvey/Enabled")] [PAMDisplayControl(SortIndex = 2)]
        ...
        [LocalizedField("PartModules/OrbitalSurvey/State")] [PAMDisplayControl(SortIndex = 4)]
        public ModuleProperty<string> State = new("");

        [LocalizedField("PartModules/OrbitalSurvey/PercentComplete")] [PAMDisplayControl(SortIndex = 5)] [JsonIgnore]
        public ModuleProperty<float> PercentComplete = new(0f, true, val => $"{val:P0}");

        [LocalizedField("PartModules/OrbitalSurvey/ScanningFOV")] [PAMDisplayControl(SortIndex = 6)] [JsonIgnore]
        public ModuleProperty<float> ScanningFieldOfView = new(0, true, val => $"{val:N0}°");

        [LocalizedField("PartModules/OrbitalSurvey/BodyCategory")] [PAMDisplayControl(SortIndex = 7)]
        public ModuleProperty<string> BodyCategory = new("");

        [LocalizedField("PartModules/OrbitalSurvey/BodyCategory/Preview")] [PAMDisplayControl(SortIndex = 7)]
        public ModuleProperty<string> BodyCategoryOabDropdown = new("");

        [LocalizedField("PartModules/OrbitalSurvey/MinAltitude")] [PAMDisplayControl(SortIndex = 8)] [JsonIgnore]
        public ModuleProperty<float> MinimumAltitude = new(0, true, val => $"{((float)val / 1000):N0} km");

        [LocalizedField("PartModules/OrbitalSurvey/IdealAltitude")] [PAMDisplayControl(SortIndex = 9)] [JsonIgnore]
        public ModuleProperty<float> IdealAltitude = new(0, true, val => $"{((float)val / 1000):N0} km");

        [LocalizedField("PartModules/OrbitalSurvey/MaxAltitude")] [PAMDisplayControl(SortIndex = 10)] [JsonIgnore]
            ...
            if (partBehaviourModuleType == ModuleType)
            {
                ...
                var entry = new OABPartData.PartInfoModuleEntry(new LocalizedString(ModeValue),
                    _ =>
                    {
                        ...
                        var mapType = LocalizationStrings.MODE_TYPE_TO_MAP_TYPE[ModeValue];
                        var allCategoryAltitudes =
                            CelestialCategoryManager.Instance.GetCategoryAltitudesForGivenMapType(mapType);
                        ...
                        foreach (var categoryAltitudes in allCategoryAltitudes)
                        {
                            ...
                            var maxRadius =
                                CelestialCategoryManager.Instance.MaxRadiusDefinition[categoryAltitudes.category] /
                                1000f;
                            var categoryDescriptionLocalized =
                                LocalizationStrings.OAB_DESCRIPTION["CategoryDescription"];
                            var categoryDescription = string.Format(categoryDescriptionLocalized,
                                categoryAltitudes.category, maxRadius.ToString("N0"));

                            var categorySubEntry =
                                new OABPartData.PartInfoModuleSubEntry(categoryDescription,
                                    categoryAltitudesSubEntries);
        ...
        public override void SetupResourceRequest(ResourceFlowRequestBroker resourceFlowRequestBroker)
        {
            if (UseResources)
            {
                ResourceDefinitionID resourceIDFromName =
                    GameManager.Instance.Game.ResourceDefinitionDatabase.GetResourceIDFromName(this.RequiredResource
                        .ResourceName);
                if (resourceIDFromName == ResourceDefinitionID.InvalidID)
                {
                    Logger.LogError($"There are no resources with name {this.RequiredResource.ResourceName}");
                    return;
                }
        ...
        [KSPDefinition] [Tooltip("Whether the module consumes resources")]
        public bool UseResources = true;

        public bool HasResourcesToOperate = true;

        [KSPDefinition] [Tooltip("Resource required to operate this module if it consumes resources")]
        public PartModuleResourceSetting RequiredResource;

        public ResourceFlowRequestCommandConfig RequestConfig;

        [JsonIgnore] public PartComponentModule_OrbitalSurvey PartComponentModule;
    }
}