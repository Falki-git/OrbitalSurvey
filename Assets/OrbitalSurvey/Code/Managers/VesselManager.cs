...
#pragma warning restore CS0067 // Event is never used
        ...
        public void RegisterModule(VesselComponent vessel, Data_OrbitalSurvey dataModule)
        {
            if (vessel == null)
            {
                Logger.LogError($"Vessel is 'null'. Registration cannot proceed!");
                return;
            }
            ...
            {
                Logger.LogError(
                    $"Data_OrbitalSurvey module for Vessel {vessel.Name} is 'null'. Registration cannot proceed!");
                return;
            }
            ...
            if (isNewVessel)
            {
                ...
                Logger.LogInfo(
                    $"New vessel '{vesselStats.Name}' registered. Number of modules: {vesselStats.ModuleStats.Count}.");
            }
        }
        ...
        public void UnRegisterModule(VesselComponent vessel, Data_OrbitalSurvey dataModule)
        {
            if (vessel == null)
            {
                Logger.LogError($"Vessel is null! Unregistration will not continue.");
                return;
            }
            ...
            if (foundVessel == null)
            {
                Logger.LogError($"Vessel '{vessel.Name}' is not registered! Unregistration will not continue.");
                return;
            }
            ...
            {
                Logger.LogError(
                    $"Data_OrbitalSurvey module on vessel '{vessel.Name}' is not registered! Unregistration will not continue.");
                return;
            }
            ...
            if (foundVessel.ModuleStats.Count > 1)
            {
                ...
                Logger.LogInfo($"Data_OrbitalSurvey module on vessel '{vessel.Name}' has been unregistered.");
            }
            else
            {
        ...
        public void UnRegisterVessel(VesselStats vesselStats)
        {
            ...
            Logger.LogInfo($"Vessel '{vesselStats.Name}' has been unregistered.");
        }
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < OrbitalSurveyVessels.Count; i++)
            {
                ...
                if (vesselStats.Body != vesselStats.Vessel.Orbit.referenceBody.bodyName)
                {
                    ...
                    Logger.LogInfo($"Vessel '{vesselStats.Name}' changed reference body to '{vesselStats.Body}'.");
                }
                ...
                for (int j = 0; j < vesselStats.ModuleStats.Count; j++)
                {
                    ...
                    var moduleVessel = moduleStats.DataModule.PartComponentModule.Part.PartOwner.SimulationObject
                        .Vessel;
        ...
        public class VesselStats
        {
            ...
            public delegate void
                GeographicCoordinatesChanged((double Latitude, double Longitude) geographicCoordinates);
