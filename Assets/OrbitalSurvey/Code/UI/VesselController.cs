    ...
    public class VesselController : MonoBehaviour
    {
        ...
        private IEnumerator Rebuild(string body)
        {
            ...
            foreach (var vessel in vessels)
            {
                ...
                vessel.OnMapGuiPositionChanged +=
                    (value) => OnMapGuiPositionChanged(control, value, vessel.IsActiveVessel);
        ...
        private void OnMapGuiPositionChanged(MapMarkerControl control,
            (float percentX, float percentY) mapGuiPositionChanged, bool isActiveVessel)
        {
            var scaledCoordinates =
                GetScaledCoordinates(mapGuiPositionChanged.percentX, 1 - mapGuiPositionChanged.percentY);
        ...
        private void OnGeographicCoordinatesChanged(MapMarkerControl control,
        ...
        private void RepositionAllVesselControls()
        {
            foreach (var vesselTuple in _trackedVessels)
            {
                ...
                var scaledCoordinates = GetScaledCoordinates(vessel.MapLocationPercent.PercentX,
                    1 - vessel.MapLocationPercent.PercentY);
                control.style.left = scaledCoordinates.x;
                control.style.top = scaledCoordinates.y;
            }
        }
