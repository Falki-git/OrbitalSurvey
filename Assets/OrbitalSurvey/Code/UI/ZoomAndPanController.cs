    ...
    public class ZoomAndPanController : MonoBehaviour
    {
        ...
        public void OnPanMoving(PointerMoveEvent evt)
        {
            if (_isPanning)
            {
                ...
                var maxPanOffset = new Vector2((canvasWidth - canvasWidth / ZoomFactor) / 2,
                    (canvasHeight - canvasHeight / ZoomFactor) / 2);
                ...
                _mapContainer.transform.position =
                    new Vector3(newPanOffset.x * ZoomFactor, newPanOffset.y * ZoomFactor, 0);
        ...
        private void PanCanvasToActiveVesselPosition((float percentX, float percentY) vesselPosition)
        {
            ...
            var newPosition = new Vector2(
                -1 * (_mapContainer.layout.width * vesselPosition.percentX - _mapContainer.layout.width / 2) *
                ZoomFactor,
                -1 * (_mapContainer.layout.height * (1 - vesselPosition.percentY) - _mapContainer.layout.height / 2) *
                ZoomFactor
            );
